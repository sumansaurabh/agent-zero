...
> **注意：** Agent Zero 不使用 Twitter/X。任何声称代表此项目的 Twitter/X 帐户均为虚假帐户。
...
> [!注意]
> **🎉 v0.8.1 发布**: 现在推出了一款能够使用 Chromium 进行网页交互的浏览器代理！这使得 Agent Zero 能够自主浏览网页、收集信息并与网页内容进行交互。


https://github.com/user-attachments/assets/c168759d-57d8-4b43-b62a-1026afcf52e6

## 一个与您一起成长和学习的个人、有机代理框架

- Agent Zero 不是一个预定义的代理框架。它旨在动态地、有机地随着您的使用而成长和学习。
- Agent Zero 是完全透明、可读、可理解、可定制和可交互的。
...
- 它具有持久性内存，允许它记住以前的解决方案、代码、事实、指令等，以便将来更快、更可靠地解决任务。
...
- 其唯一的默认工具是在线搜索、内存功能、通信（与用户和其他代理）以及代码/终端执行。其他所有内容都由代理本身创建，或者可以由用户扩展。
- 工具使用功能已从头开始开发，即使是小型模型也具有最高的兼容性和可靠性。
...
- 每个代理都有一个上级代理为其提供任务和指令。然后每个代理向上级报告。
- 在链中的第一个代理（Agent 0）的情况下，上级是人类用户；代理没有发现任何区别。
...
- 此框架中几乎没有任何内容是硬编码的。没有任何东西是隐藏的。一切都可以由用户扩展或更改。
- 整个行为由 **prompts/default/agent.system.md** 文件中的系统提示定义。更改此提示可以极大地改变框架。
- 框架不会以任何方式引导或限制代理。代理没有必须遵循的硬编码轨道。
- 代理通信循环中发送给代理的每个提示、每个小消息模板都可以在 **prompts/** 文件夹中找到并更改。
...
- 代理可以与它们的上级和下级通信，提出问题、给出指令和提供指导。在系统提示中指导您的代理如何有效沟通。
- 终端界面实时流式传输且具有交互性。您可以随时停止并干预。如果您发现您的代理方向错误，请立即停止并告诉它。
- 这个框架有很多自由。您可以指示您的代理定期向上级报告，请求继续的许可。您可以指示它们在决定何时委托子任务时使用积分系统。上级可以复查下级的成果并提出异议。可能性是无限的。
...
Windows、macOS 和 Linux 的详细设置指南（含视频）可在 Agent Zero 文档的 [此页面](./docs/installation.md) 中找到。
...
- 可自定义设置允许用户根据自己的需求调整代理的行为和响应。
- Web UI 输出非常清晰、流畅、多彩、可读且具有交互性；没有任何内容被隐藏。
...
- 无需编码；只需提示和沟通技巧。
- 凭借可靠的系统提示，即使是小型模型，框架也可靠，包括精确的工具使用。

## 👀 请记住

1. **Agent Zero 可能很危险！**

- 通过适当的指令，Agent Zero 能够做很多事情，甚至可能对您的计算机、数据或帐户造成潜在危险。始终在隔离环境（如 Docker）中运行 Agent Zero，并小心您的愿望。
