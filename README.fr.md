...
> **Note :** Agent Zero n'utilise pas Twitter/X. Tous les comptes Twitter/X prétendant représenter ce projet sont des faux.
...
> [!NOTE]
> **🎉 Version v0.8.1** : Désormais doté d'un agent navigateur capable d'utiliser Chromium pour les interactions web ! Cela permet à Agent Zero de naviguer sur le web, de recueillir des informations et d'interagir avec le contenu web de manière autonome.
...
- Agent Zero n'est pas un framework agentique prédéfini. Il est conçu pour être dynamique, en croissance organique, et pour apprendre au fur et à mesure que vous l'utilisez.
...
- Agent Zero n'est pas pré-programmé pour des tâches spécifiques (mais peut l'être). Il est destiné à être un assistant personnel polyvalent. Donnez-lui une tâche, et il recueillera des informations, exécutera des commandes et du code, coopérera avec d'autres instances d'agents, et fera de son mieux pour l'accomplir.
- Il possède une mémoire persistante, lui permettant de mémoriser les solutions précédentes, le code, les faits, les instructions, etc., pour résoudre les tâches plus rapidement et plus fiablement à l'avenir.
...
- Les seuls outils par défaut dans son arsenal sont la recherche en ligne, les fonctionnalités de mémoire, la communication (avec l'utilisateur et les autres agents) et l'exécution de code/terminal. Tout le reste est créé par l'agent lui-même ou peut être étendu par l'utilisateur.
- La fonctionnalité d'utilisation des outils a été développée from scratch pour être la plus compatible et fiable, même avec de très petits modèles.
...
- Chaque agent a un agent supérieur qui lui donne des tâches et des instructions. Chaque agent fait ensuite rapport à son supérieur.
...
- Chaque agent peut créer son agent subordonné pour aider à décomposer et résoudre des sous-tâches. Cela aide tous les agents à garder leur contexte propre et focalisé.
...
- Donnez à votre agent un bon prompt système et des instructions, et il peut faire des miracles.
...
- L'interface terminal est diffusée en temps réel et interactive. Vous pouvez arrêter et intervenir à tout moment. Si vous voyez votre agent partir dans la mauvaise direction, arrêtez-le simplement et dites-le-lui immédiatement.
- Il y a beaucoup de liberté dans ce framework. Vous pouvez instruire vos agents de faire régulièrement rapport aux supérieurs en demandant la permission de continuer. Vous pouvez leur demander d'utiliser des systèmes de notation par points pour décider quand déléguer des sous-tâches. Les supérieurs peuvent vérifier les résultats des subordonnés et contester. Les possibilités sont infinies.
...
- L'interface Web UI est très propre, fluide, colorée, lisible et interactive ; rien n'est caché.
- Vous pouvez charger ou sauvegarder des conversations directement dans l'interface Web UI.
...
- Aucune programmation n'est requise ; seules des compétences en prompting et en communication sont nécessaires.
- Avec un bon prompt système, le framework est fiable même avec de petits modèles, y compris l'utilisation précise des outils.
...
2. **Agent Zero est basé sur les prompts.**

- L'ensemble du framework est guidé par le dossier **prompts/**. Les directives de l'agent, les instructions des outils, les messages, les fonctions IA utilitaires, tout est là.
...
- **Page de paramètres dans l'interface Web UI**
