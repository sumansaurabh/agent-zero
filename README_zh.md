...
> [!NOTE]
> **🎉 v0.8.1 版本发布**：现在配备了能够使用 Chromium 进行网页交互的浏览器代理！这使得 Agent Zero 能够自主浏览网页、收集信息并与网页内容进行交互。


https://github.com/user-attachments/assets/c168759d-57d8-4b43-b62a-1026afcf52e6

## 一个与你一起成长和学习的个人化、有机智能体框架

- Agent Zero 不是一个预定义的智能体框架。它被设计为动态的、有机成长的，并随着你的使用而学习。
- Agent Zero 完全透明、可读、可理解、可定制且可交互。
- Agent Zero 将计算机作为工具来完成它的（你的）任务。

# 💡 核心特性

1. **通用助手**

- Agent Zero 不是为特定任务预编程的（但可以这样做）。它旨在成为一个通用的个人助手。给它一个任务，它会收集信息、执行命令和代码、与其他智能体实例合作，并尽最大努力完成任务。
...
- 它的默认工具库中只有在线搜索、记忆功能、通信（与用户和其他智能体）以及代码/终端执行。其他一切都由智能体自己创建，或者可以由用户扩展。
- 工具使用功能是从头开发的，即使是非常小的模型也能实现最佳兼容性和可靠性。
- **默认工具：** Agent Zero 包含知识、网页内容、代码执行和通信等工具。
- **创建自定义工具：** 通过创建自己的自定义工具来扩展 Agent Zero 的功能。
- **仪器：** 仪器是一种新型工具，允许你创建可由 Agent Zero 调用的自定义函数和过程。

3. **多智能体协作**

- 每个智能体都有一个上级智能体给它分配任务和指令。然后每个智能体向其上级报告。
- 对于链中的第一个智能体（Agent 0），上级是人类用户；智能体看不出区别。
...
- 整个行为由 **prompts/default/agent.system.md** 文件中的系统提示定义。更改此提示可以显著改变框架。
- 框架不会以任何方式引导或限制智能体。没有智能体必须遵循的硬编码规则。
- 每个提示、在通信循环中发送给智能体的每个小消息模板都可以在 **prompts/** 文件夹中找到并更改。
...
- 智能体可以与其上级和下级沟通，提出问题、给出指令和提供指导。在系统提示中指导你的智能体如何有效沟通。
- 终端界面是实时流式传输和交互式的。你可以随时停止和干预。如果你看到你的智能体朝着错误的方向前进，只需立即停止并告诉它。
- 这个框架有很大的自由度。你可以指示你的智能体定期向上级报告，请求继续的许可。你可以指示它们在决定何时委派子任务时使用评分系统。上级可以仔细检查下级的结果并提出异议。可能性是无限的。
...
Windows、macOS 和 Linux 的详细设置指南（附视频）可以在 Agent Zero 文档的[此页面](./docs/installation.md)找到。
...
- 开发者和贡献者：从[发布页面](https://github.com/frdel/agent-zero/releases)下载适合你系统的完整二进制文件，然后按照[此处提供的说明](./docs/installation.md#in-depth-guide-for-full-binaries-installation)操作。

## 🐳 完全 Docker 化，支持语音转文本和文本转语音

![设置](docs/res/settings-page-ui.png)

- 可定制的设置允许用户根据需要调整智能体的行为和响应。
- Web UI 输出非常干净、流畅、多彩、可读且可交互；没有任何隐藏内容。
- 你可以直接在 Web UI 中加载或保存聊天记录。
- 你在终端中看到的相同输出会自动保存到 **logs/** 文件夹中的 HTML 文件，用于每个会话。

![时间示例](/docs/res/time_example.jpg)

- 智能体输出实时流式传输，允许用户随时阅读并干预。
- 无需编码；只需要提示和沟通技巧。
- 有了可靠的系统提示，即使是小型模型，框架也很可靠，包括精确的工具使用。

## 👀 注意事项

1. **Agent Zero 可能很危险！**

- 通过适当的指令，Agent Zero 能够做很多事情，甚至可能对你的计算机、数据或账户采取潜在危险的操作。始终在隔离环境（如 Docker）中运行 Agent Zero，并小心你的要求。
...
> [!IMPORTANT]
>
>**自 v0.7 以来 frdel/agent-zero Docker 镜像的变化：**
>
