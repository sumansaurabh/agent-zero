...
> **注意：** Agent Zero 不使用 Twitter/X。任何声称代表本项目的 Twitter/X 账户都是假冒的。
...
> [!NOTE]
> **🎉 v0.8.1 版本发布**：现在具备浏览器代理功能，能够使用 Chromium 进行网页交互！这使得 Agent Zero 能够自主浏览网页、收集信息并与网页内容进行交互。


https://github.com/user-attachments/assets/c168759d-57d8-4b43-b62a-1026afcf52e6

## 一个与你共同成长和学习的个人化、有机的智能体框架
...
- Agent Zero 不是为特定任务预编程的（但可以）。它旨在成为一个通用的个人助手。给它一个任务，它会收集信息、执行命令和代码、与其他智能体实例合作，并尽最大努力完成任务。
...
- 工具使用功能是从头开发的，即使是非常小的模型也具有最佳的兼容性和可靠性。
...
- 这个框架中几乎没有硬编码的内容。没有任何隐藏。用户可以扩展或更改一切。
- 整个行为由 **prompts/default/agent.system.md** 文件中的系统提示定义。更改此提示可以显著改变框架。
- 该框架不以任何方式引导或限制智能体。没有智能体必须遵循的硬编码规则。
...
- 开发者和贡献者：从[发布页面](https://github.com/frdel/agent-zero/releases)下载适用于你系统的完整二进制文件，然后按照[此处提供的说明](./docs/installation.md#in-depth-guide-for-full-binaries-installation)进行操作。
...
- Web UI 输出非常干净、流畅、丰富多彩、可读且可交互；没有任何隐藏。
- 你可以直接在 Web UI 中加载或保存聊天记录。
- 你在终端中看到的相同输出会自动保存到 **logs/** 文件夹中的 HTML 文件中，用于每个会话。

![时间示例](/docs/res/time_example.jpg)

- 智能体输出实时流式传输，允许用户随时阅读并干预。
- 无需编码；只需要提示和沟通技巧。
- 通过可靠的系统提示，即使是小型模型，该框架也很可靠，包括精确的工具使用。

## 👀 注意事项

1. **Agent Zero 可能很危险！**

- 通过适当的指令，Agent Zero 能够做很多事情，甚至可能对你的计算机、数据或账户采取潜在危险的操作。始终在隔离环境（如 Docker）中运行 Agent Zero，并小心你的愿望。
